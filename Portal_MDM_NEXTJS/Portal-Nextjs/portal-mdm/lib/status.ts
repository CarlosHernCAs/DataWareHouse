...
/**
 * Clase tailwind para `border-left` tonal. Centraliza el patrón
 * `border-l-4 + border-l-[var(--color-…)]` que antes vivía duplicado en
 * `dashboard-card-frame.tsx` y `active-run-card.tsx`. Si pasas `undefined`
 * cae al borde neutro.
 */
export function toneBorderLeftClass(tone: Tone | undefined): string {
  switch (tone) {
    case "critical": return "border-l-[var(--color-destructive)]";
    case "warning":  return "border-l-[var(--color-warning)]";
    case "ok":       return "border-l-[var(--color-success)]";
    case "info":     return "border-l-[var(--color-info)]";
    case "neutral":  return "border-l-[var(--color-text-muted)]";
