...
function statsFor(
  ...
  const stats: LoadStats = { ...EMPTY_STATS, historyDurations: [] };
  ...
  for (const c of corridas) {
    ...
    if (isSuccess(c.estado)) {
      if (recent) stats.hadSuccessRecent = true;
      if (c.duracion_segundos != null) {
        rawDurations.push(c.duracion_segundos);
      }
    }
  }
  // Los más recientes están primero, tomamos 5 y los invertimos
  stats.historyDurations = rawDurations.slice(0, 5).reverse();
  return stats;
}
...
export async function GET() {
  ...
  function buildNode(
    ...
    let agg: LoadStats = { ...EMPTY_STATS, historyDurations: [] };
    if (layer === "silver") {
      ...
      for (const factName of factsForTable) {
        ...
        // Solo para bronce/gold mezclamos duraciones de los facts que las tocan,
        // no tiene mucho sentido estadístico estricto, pero al menos muestra algo
        if (s.historyDurations.length > 0 && agg.historyDurations.length === 0) {
          agg.historyDurations = s.historyDurations;
        }
      }
    }
    return {
      ...
      historyDurations: agg.historyDurations,
    };
  }
  ...
  const factSummaries: FactSummary[] = facts
    .map((f) => {
      ...
      return {
        ...
        historyDurations: s.historyDurations,
      };
    })
