...
import { DwhPreviewDialog } from "@/components/control-center/dwh-preview-dialog";
import {
  DwhNodeContextMenu,
  type NodeMenuItem,
...
export function DwhExplorerClient() {
  ...
  const matchCount = useMemo(() => {
    ...
    return data.nodes.filter((n) => {
      ...
      if (!url.layers.has(n.layer)) return false;
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <Toolbar
        ...
        layers={url.layers}
        onToggleLayer={url.toggleLayer}
        onClearLayers={url.clearLayers}
        collapsedLayers={url.collapsed}
        onExpandAllLayers={url.expandAllLayers}
        ...
        >
          ...
          {selectedNode && compareNode ? (
            ...
            <DwhFactPanel
              ...
              onPreview={() => setPreviewTable(selectedNode.fullName)}
              onCompare={() => {
        ...
        <DwhNodeContextMenu
          ...
          items={buildContextItems({
            ...
            onPreview: () => {
              ...
              if (n) setPreviewTable(n.fullName);
            },
            onLaunch: (fact) =>
...
function buildContextItems(p: ContextItemsParams): NodeMenuItem[] {
  ...
  return [
    ...
    {
      id: "preview",
      label: "Vista previa (TOP 10)",
      icon: <Database className="h-3.5 w-3.5" />,
      onSelect: p.onPreview,
    },
...
interface ToolbarProps {
  ...
  onClearStatus: () => void;
  layers: Set<DwhLayer>;
  onToggleLayer: (l: DwhLayer) => void;
  onClearLayers: () => void;
  collapsedLayers: Set<DwhLayer>;
  onExpandAllLayers: () => void;
  hasFilters: boolean;
  onResetAll: () => void;
  matchCount: number;
  totalCount: number;
    ...
    <div className="flex flex-col gap-3">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          {p.collapsedLayers.size > 0 ? (
            <button
              type="button"
              onClick={p.onExpandAllLayers}
              className="inline-flex h-7 items-center gap-1.5 rounded-md border border-[var(--color-border)] bg-[var(--color-surface-2)] px-2 text-xs text-[var(--color-text-secondary)] transition hover:text-[var(--color-text)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-ring)]"
            >
      ...
      <div className="flex flex-col gap-2">
        ...
        >
          <span className="text-xs text-[var(--color-text-muted)] w-12">Capas:</span>
          {DWH_ALL_LAYERS.map((l) => {
            const active = p.layers.has(l);
            return (
              <button
                key={l}
                type="button"
                aria-pressed={active}
                onClick={() => p.onToggleLayer(l)}
                className={cn(
                  "transition-opacity",
                  p.layers.size === DWH_ALL_LAYERS.length ? "opacity-100" : active ? "opacity-100" : "opacity-50",
                )}
              >
                <span className="inline-flex items-center rounded-md border border-[var(--color-border)] bg-[var(--color-surface-2)] px-2 py-0.5 text-xs font-medium text-[var(--color-text)] hover:bg-[var(--color-surface)]">
                  {l.charAt(0).toUpperCase() + l.slice(1)}
                </span>
              </button>
            );
          })}
          {p.layers.size !== DWH_ALL_LAYERS.length ? (
            <button
              type="button"
              onClick={p.onClearLayers}
        ...
        <div
          ...
          className="flex flex-wrap items-center gap-2"
        >
          <span className="text-xs text-[var(--color-text-muted)] w-12">Estados:</span>
