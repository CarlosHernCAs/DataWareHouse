...
def _render_filtro(df: pd.DataFrame, col_nombre: str, col_breeder: str, clave: str, mostrar_estado: bool = False) -> pd.DataFrame:
    with st.container(border=True):
        ...
        with cols[1]:
            breeders = ["Todas las casas"] + sorted(df[col_breeder].dropna().unique().tolist()) \
                if col_breeder in df.columns else ["Todas las casas"]
        ...
        if mostrar_estado and len(cols) > 2:
            with cols[2]:
                estado_sel = st.selectbox(
                    "Estado", ["Todos los estados", "Activas", "Desactivadas"],
    ...
    if breeder_sel != "Todas las casas" and col_breeder in dff.columns:
