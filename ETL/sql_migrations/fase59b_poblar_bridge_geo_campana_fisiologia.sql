-- =============================================================================
-- fase59b_poblar_bridge_geo_campana_fisiologia.sql
-- =============================================================================
-- Pobla Silver.Bridge_Geografia_Campana desde Silver.Fact_Fisiologia.
...
--   - fase59a (backfill ID_Campana) aplicado.
--
...
BEGIN
    RAISERROR('Silver.Fact_Fisiologia no existe.', 16, 1);
    RETURN;
END;
GO
...
-- -----------------------------------------------------------------------------
CREATE OR ALTER PROCEDURE MDM.usp_Popular_Bridge_Geografia_Campana_Fisiologia
AS
BEGIN
    SET NOCOUNT ON;

    IF OBJECT_ID('tempdb..#Combinaciones_Fisiologia') IS NOT NULL DROP TABLE #Combinaciones_Fisiologia;
    ...
    -- Cuarentena FK
    IF OBJECT_ID('tempdb..#Cuarentena_FK_Fisiologia') IS NOT NULL DROP TABLE #Cuarentena_FK_Fisiologia;
    SELECT
        c.ID_Geografia,
        c.ID_Campana,
        c.Filas_Fact,
        ...
        END
    INTO #Cuarentena_FK_Fisiologia
      FROM #Combinaciones_Fisiologia c
    ...
    DECLARE @cuar INT = (SELECT COUNT(*) FROM #Cuarentena_FK_Fisiologia);
    IF @cuar > 0
    BEGIN
        PRINT CONCAT('Cuarentena FK invalida: ', @cuar, ' combinaciones.');
        SELECT TOP 20 * FROM #Cuarentena_FK_Fisiologia ORDER BY Filas_Fact DESC;
    END;
    ...
    PRINT CONCAT('Bridge_GC (Fisiologia): total filas bridge = ',
...
PRINT '=== C. Cobertura de Fact_Fisiologia ===';
;WITH F AS (
    SELECT DISTINCT
        f.ID_Geografia,
        ISNULL(MDM.fn_Resolver_ID_Campana_Por_Fecha(CAST(f.Fecha_Evento AS DATE)), 0) AS ID_Campana
...
PRINT 'fase59b: Bridge_GC poblado desde Fisiologia.';
GO
