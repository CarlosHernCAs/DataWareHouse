-- =============================================================================
-- fase57a_censo_plantas_id_campana_backfill.sql
-- =============================================================================
-- Objetivo:
--   Llevar Silver.Fact_Censo_Plantas al ecosistema Geografia-Campana.
--   La columna ID_Campana ya existe pero esta NULL para filas historicas.
--
-- Nota: Silver.Fact_Censo_Plantas NO tiene columna Estado_DQ
--   (el loader Python aun no esta operativo por mapeo Raw pendiente de
--   definicion - Task fase35). El backfill cubre todas las filas existentes.
--
-- Acciones:
...
-- 1. Auditoria pre-backfill
-- -----------------------------------------------------------------------------
PRINT '=== 1. Estado pre-backfill de Silver.Fact_Censo_Plantas ===';
SELECT
    Filas_Total         = COUNT(*),
  ...
  FROM Silver.Fact_Censo_Plantas;
GO
...
-- -----------------------------------------------------------------------------
-- 3. Backfill (idempotente, sin filtro Estado_DQ - la tabla no tiene esa columna)
-- -----------------------------------------------------------------------------
...
PRINT CONCAT('fase57a: filas actualizadas = ', @upd);
GO
...
SELECT
    ...
    Resultado       = CASE
        WHEN SUM(CASE WHEN ID_Campana IS NULL THEN 1 ELSE 0 END) = 0
            THEN 'OK: backfill completo (o tabla vacia)'
        ELSE 'FAIL: aun hay NULLs'
    END
  FROM Silver.Fact_Censo_Plantas;
GO

PRINT 'fase57a: backfill ID_Campana en Fact_Censo_Plantas completado.';
GO
