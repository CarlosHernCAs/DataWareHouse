-- =============================================================================
-- fase57b_poblar_bridge_geo_campana_censo_plantas.sql
-- =============================================================================
-- Pobla Silver.Bridge_Geografia_Campana desde Silver.Fact_Censo_Plantas.
-- Reutiliza la MISMA tabla bridge global (sin condicion).
--
-- Nota: Fact_Censo_Plantas no tiene columna Estado_DQ. Se toman todas las
--   filas con ID_Geografia e Fecha_Evento validos.
--
-- Pre-requisitos:
--   - fase48a (resolver) aplicado.
--   - fase48b (tabla Bridge_Geografia_Campana) aplicado.
--   - fase57a (backfill ID_Campana) aplicado.
--
...
BEGIN
    RAISERROR('Silver.Fact_Censo_Plantas no existe.', 16, 1);
    RETURN;
END;
GO
...
-- -----------------------------------------------------------------------------
CREATE OR ALTER PROCEDURE MDM.usp_Popular_Bridge_Geografia_Campana_Censo_Plantas
AS
BEGIN
    SET NOCOUNT ON;

    IF OBJECT_ID('tempdb..#Combinaciones_CensoPlantas') IS NOT NULL DROP TABLE #Combinaciones_CensoPlantas;

    -- Sin filtro Estado_DQ (la tabla no tiene esa columna)
    SELECT
        f.ID_Geografia,
        ...
        COUNT(*)                                                                       AS Filas_Fact
    INTO #Combinaciones_CensoPlantas
      FROM Silver.Fact_Censo_Plantas f
     WHERE f.ID_Geografia IS NOT NULL
    ...
    -- Cuarentena FK
    IF OBJECT_ID('tempdb..#Cuarentena_FK_CensoPlantas') IS NOT NULL DROP TABLE #Cuarentena_FK_CensoPlantas;
    SELECT
        c.ID_Geografia,
        c.ID_Campana,
        c.Filas_Fact,
        ...
        END
    INTO #Cuarentena_FK_CensoPlantas
      FROM #Combinaciones_CensoPlantas c
    ...
    DECLARE @cuar INT = (SELECT COUNT(*) FROM #Cuarentena_FK_CensoPlantas);
    IF @cuar > 0
    BEGIN
        PRINT CONCAT('Cuarentena FK invalida: ', @cuar, ' combinaciones.');
        SELECT TOP 20 * FROM #Cuarentena_FK_CensoPlantas ORDER BY Filas_Fact DESC;
    END;
    ...
    PRINT CONCAT('Bridge_GC (Censo_Plantas): total filas bridge = ',
...
PRINT '=== C. Cobertura de Fact_Censo_Plantas ===';
;WITH F AS (
    SELECT DISTINCT
        f.ID_Geografia,
        ISNULL(MDM.fn_Resolver_ID_Campana_Por_Fecha(CAST(f.Fecha_Evento AS DATE)), 0) AS ID_Campana
...
PRINT 'fase57b: Bridge_GC poblado desde Censo_Plantas.';
GO
