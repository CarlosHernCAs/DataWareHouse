-- =============================================================================
-- fase52b_poblar_bridge_geo_campana_conteo_fenologico.sql
-- =============================================================================
-- Pobla Silver.Bridge_Geografia_Campana desde Silver.Fact_Conteo_Fenologico
-- usando el resolver oficial. Reutiliza la MISMA tabla bridge que Vegetativa
-- (sin condicion): es global, no por-fact.
--
-- Pre-requisitos:
--   - fase48a (resolver) aplicado.
--   - fase48b (tabla Bridge_Geografia_Campana) aplicado.
--   - fase52a (backfill ID_Campana en el fact) aplicado.
--
...
-- -----------------------------------------------------------------------------
CREATE OR ALTER PROCEDURE MDM.usp_Popular_Bridge_Geografia_Campana_Conteo_Fenologico
AS
BEGIN
    SET NOCOUNT ON;

    -- Combinaciones distintas (Geografia, Campana_resuelta) con vigencia
    -- derivada de las fechas observadas en el fact.
    IF OBJECT_ID('tempdb..#Combinaciones_ConteoFen') IS NOT NULL DROP TABLE #Combinaciones_ConteoFen;
    ...
    -- Cuarentena FK: pares (Geo, Camp) con dimensiones inexistentes
    IF OBJECT_ID('tempdb..#Cuarentena_FK_ConteoFen') IS NOT NULL DROP TABLE #Cuarentena_FK_ConteoFen;
    SELECT
        c.ID_Geografia,
        c.ID_Campana,
        c.Filas_Fact,
        ...
        END
    INTO #Cuarentena_FK_ConteoFen
      FROM #Combinaciones_ConteoFen c
    ...
    DECLARE @cuar INT = (SELECT COUNT(*) FROM #Cuarentena_FK_ConteoFen);
    IF @cuar > 0
    BEGIN
        PRINT CONCAT('Cuarentena FK invalida: ', @cuar, ' combinaciones (ver #Cuarentena_FK_ConteoFen).');
        SELECT TOP 20 * FROM #Cuarentena_FK_ConteoFen ORDER BY Filas_Fact DESC;
    END;
    ...
    WHEN MATCHED AND (
            -- Extender vigencias si el fact ahora abarca un rango mayor
    ...
    PRINT CONCAT('Bridge_GC (Conteo_Fenologico): total filas bridge = ',
                 @total_bridge,
                 ', cuarentena FK = ', @cuar);
END;
GO
...
-- -----------------------------------------------------------------------------
-- 3. Auditoria post-poblamiento (combinado con Vegetativa)
-- -----------------------------------------------------------------------------
PRINT '=== A. Total bridge GLOBAL (Vegetativa + Conteo_Fenologico) ===';
SELECT
    Filas_Bridge       = COUNT(*),
...
PRINT '=== C. Cobertura del Fact_Conteo_Fenologico ===';
;WITH F AS (
    SELECT DISTINCT
        f.ID_Geografia,
        ISNULL(MDM.fn_Resolver_ID_Campana_Por_Fecha(CAST(f.Fecha_Evento AS DATE)),0) AS ID_Campana
...
SELECT
    ...
    Pct_Cobertura     = CAST(100.0 * SUM(CASE WHEN b.ID_Bridge IS NOT NULL THEN 1 ELSE 0 END) / NULLIF(COUNT(*),0) AS DECIMAL(5,2))
  FROM F
  LEFT JOIN Silver.Bridge_Geografia_Campana b
       ON b.ID_Geografia = F.ID_Geografia
      AND b.ID_Campana   = F.ID_Campana;

PRINT 'fase52b: Bridge_GC poblado desde Conteo_Fenologico.';
GO
