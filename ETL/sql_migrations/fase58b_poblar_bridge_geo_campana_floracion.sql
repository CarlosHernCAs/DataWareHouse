-- =============================================================================
-- fase58b_poblar_bridge_geo_campana_floracion.sql
-- =============================================================================
-- Pobla Silver.Bridge_Geografia_Campana desde Silver.Fact_Floracion.
-- Reutiliza la MISMA tabla bridge global (sin condicion).
--
-- Nota: Fecha_Evento en Fact_Floracion es DATE (no DATETIME2).
--   El CAST a DATE es un no-op pero garantiza compatibilidad de firma.
--
-- Pre-requisitos:
--   - fase48a (resolver) aplicado.
--   - fase48b (tabla Bridge_Geografia_Campana) aplicado.
--   - fase58a (backfill ID_Campana) aplicado.
--
...
-- -----------------------------------------------------------------------------
CREATE OR ALTER PROCEDURE MDM.usp_Popular_Bridge_Geografia_Campana_Floracion
AS
BEGIN
    SET NOCOUNT ON;

    IF OBJECT_ID('tempdb..#Combinaciones_Floracion') IS NOT NULL DROP TABLE #Combinaciones_Floracion;
    ...
    -- Cuarentena FK
    IF OBJECT_ID('tempdb..#Cuarentena_FK_Floracion') IS NOT NULL DROP TABLE #Cuarentena_FK_Floracion;
    SELECT
        c.ID_Geografia,
        c.ID_Campana,
        c.Filas_Fact,
        ...
        END
    INTO #Cuarentena_FK_Floracion
      FROM #Combinaciones_Floracion c
    ...
    DECLARE @cuar INT = (SELECT COUNT(*) FROM #Cuarentena_FK_Floracion);
    IF @cuar > 0
    BEGIN
        PRINT CONCAT('Cuarentena FK invalida: ', @cuar, ' combinaciones.');
        SELECT TOP 20 * FROM #Cuarentena_FK_Floracion ORDER BY Filas_Fact DESC;
    END;
    ...
    PRINT CONCAT('Bridge_GC (Floracion): total filas bridge = ',
...
PRINT '=== C. Cobertura de Fact_Floracion ===';
;WITH F AS (
    SELECT DISTINCT
        f.ID_Geografia,
        ISNULL(MDM.fn_Resolver_ID_Campana_Por_Fecha(CAST(f.Fecha_Evento AS DATE)), 0) AS ID_Campana
...
PRINT 'fase58b: Bridge_GC poblado desde Floracion.';
GO
